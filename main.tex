...
OpenWSN is boot-loaded on the OpenTestbed three times where each time the MAC layer selected on of the three radio options in the OpenRadio driver and selected the respective slot-template duration. 
...
\item Reliability: measured as the end-to-end PDR of the UDP packets from each mote. The root counts all missing packets between the first and last received packets from each mote. A caveat in this metric is that if packets are lost in sequence, the loss is not detected until a subsequent packet arrives. 
...
\item Radio Tx duty cycle:  measured as ratio of total radio tics in transmit mode to the total time tics. This parameter is necessary for the estimation of the mote power consumption and consequently its battery life time based on the Tx power consumption of each radio chip.
\end{itemize}
Since the stack is expected to run on memory-constrained devices, side-effects on memory footprint are also measured in terms of two parameters :
\begin{itemize}
\item Maximum packet buffer occupancy: this reflects the maximum number of packets in the packet buffer between data samples.  Packet occupancy is affected by the PHY layer link quality through keeping the packet in memory for re-transmission and/or forwarding. Therefore, it reflects potential packet drop for memory limitations. 
\item Neighbor table size: this reflects the total number of neighbors registered in the neighbor table at the time the sample is collected. This parameter is affected by PHY layer link budget because neighbors registered if a packet has been received with RSSI > -80dB. Therefore, the higher the link budget, the higher the chance that more neighbors are discovered. This parameter is significant because there is no standardized mechanism to accept any new neighbor (even if it is a "better" one) in case neighbor table is saturated -- as outlines in the IETF RFC \cite{p.thuberted.00registration}. Therefore, neighbor table overflow could avert motes from picking the best neighbor in the vicinity.
\end{itemize}
...
Network formation time  is evaluated as the time elapsed until reception of the first packet  from all motes in the network.
...
Since all motes are boot-loaded at the same time, this metric would reflect the worst case scenario for the network formation speed after installation. 
