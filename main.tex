...
\item End-to-End Reliability: refers to the ratio of packets transmitted all motes to the packets delivered at the root. 
...
The first packet is received from  90\% of the motes within 6, 8, and 10 minutes with \fsk\ , \ofdm\ , and \oqpsk\ respectively. 
The network forms faster with higher link budget PHY layer for two reasons. First, nodes discover more neighbors and therefore  it takes less time to hear a synchronized node. Second, nodes have higher chance of finding a parent closer to the root (i.e. with lower DAG rank), if not the root itself and this decreases the number of hops necessary for an end-to-end packet delivery.
...
The impact of the PHY layer on neighbor reach can be observed in fig. \ref{fig:neighbors_all}: the median and inter-quartile range of discovered neighbors (fig. \ref{fig:neighbors_time}) accelerate faster in an inverse correlation with link budget.
...
It can be observed that the range of \ofdm\ overlaps with \fsk\ even though it is 16 times faster in bit-rate; also it is better than \oqpsk\ by nearly 35\%  even though it is 3 times faster in bit-rate. 
This observation is verified in fig. \ref{fig:neighbors_cdf} as the CDF of all the data samples in steady state shows the inverse correlation between the number of neighbors and the link budget. 
...
This risk is significant because there is no standardized mechanism to accept any new neighbor (even if it is a "better" one) in case neighbor table is saturated -- as outlines in the IETF RFC \cite{p.thuberted.00registration}. 
...
Second, a higher link budget at the PHY layer leads to an improved DAG rank and shorter paths to the root. 
...
%measured as the end-to-end PDR of the UDP packets from each mote. The root counts all missing packets between the first and last received packets from each mote. A caveat in this metric is that if packets are lost in sequence, the loss is not detected until a subsequent packet arrives. 
...
%- Why is it affected by PHY layer? 
\begin{figure} [h]
	\centering
	\includegraphics[width=0.9\columnwidth]{pdr_pdf_plot_full_steady_normalized}
	\caption{PDF of end-to-end PDR. }
    \label{fig:pdf_pdr_plot}
\end{figure} 


% Reason 1 only OQPSK interference with WiFi (cite)

%  reason 2 and packet queue overflow (show queue plot) 
% why is this important? memory limitations, packet loss simply due to unavailable buffer space. 

\begin{figure}[h]
	\centering
	\includegraphics[width=0.90\columnwidth]{maxBufferSize_cdf_plot_full_steady}
	\caption{CDF of packet queue occupancy at steady state}
...
This is a significant KPI for IIoT since some traffic is generated by alarm events.
...
Furthermore, 90\% of the reports are late by nearly 20, 30, and 40 seconds for \fsk\ , \ofdm\ , and \oqpsk\ respectively, as observed in Fig.\ref{fig:latency_cdf}.
...
This phenomena is a directly related to the increase in path-costs in the network (expressed by the DAG rank).
...
Therefore, this leads directly to an increase in end-to-end latency distribution, especially for the nodes at the absolute bottom of the routing tree. 

\begin{figure*}[t]
	\centering
	\begin{subfigure}{0.6\columnwidth}
	\centering
	\includegraphics[width=1\columnwidth]{avg_latency_plot}
    \subcaption{Median and inter-quartile range of measurements in the time domain }
    \label{fig:latency_time}
	\end{subfigure}
	\begin{subfigure}{0.6\columnwidth}
		\centering
    	\includegraphics[width=01\columnwidth]{latency_cdf_plot_full_all}
	...
	\begin{subfigure}{0.6\columnwidth}
		\centering
	\includegraphics[width=1\columnwidth]{latency_pdf_plot_full_all_normalized}
...
% measured as ratio of total radio tics to the total time tics. This parameter is necessary for the estimation of the mote power consumption in idle and receive state. Current withdraw is considered approximately the same in both states for the three radio configurations.  

\begin{figure*}[t]
	\centering
	\begin{subfigure}{0.6\columnwidth}
	\centering
	\includegraphics[width=1\columnwidth]{avg_avg_dutyCycle_plot}
    \subcaption{Median and inter-quartile range of measurements in the time domain }
    \label{fig:dutyCycle_time}
	\end{subfigure}
	\begin{subfigure}{0.6\columnwidth}
		\centering
    	\includegraphics[width=01\columnwidth]{dutyCycle_cdf_plot_full_all}
    	\subcaption{CDF of  the measurements for all the network run }
	...
	\begin{subfigure}{0.6\columnwidth}
		\centering
	\includegraphics[width=1\columnwidth]{dutyCycle_pdf_plot_full_all_normalized}
	\subcaption{PDF of  the measurements for all the network run }
	\label{fig:dutyCycle_pdf}
	\end{subfigure}%

	\caption{Reported radio duty cycle in the network. Lower bit-rate and SPI communications overhead leads to higher duty cycle and power consumption.} 
	\label{fig:latency_all}
\end{figure*}
