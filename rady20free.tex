...
Authors demonstrate that MR-OFDM can  tolerate 15~dB of interference more than MR-FSK.
        ...
        \begin{itemize}
            ...
            \item Using \ofdm\ yields
                ...
                In line with~\cite{munoz18evaluation}, a node using \ofdm\ discovers number of neighbors closer to \fsk\ than \oqpsk\,
                    despite having the highest data rate. This is due to higher  link budget and lower vulnerability to Wi-Fi interference than \oqpsk.
        \end{itemize}
...
The duration of SPI transactions, the datarate of the communication or the turn-around time of the radio all influence the timeslot template.
...
Red markers highlight the approximate time for SPI communications (none for the on-chip CC2538 radio).
...
Each time, we load the firmware onto the testbed, the switch on then OpenVisualizer software, which connects to all motes over their serial port (over MQTT).
...
\begin{itemize}
    ...
    \item $T_{on}$ how long the sender's radio has been on receiving since the previous data packet transmission,
    \item $T_{TX}$ how long the sender's radio has been on transmitting since the previous data packet transmission,
