...
\abstract{
\thomas{will work on last}
The 6TiSCH protocol stack is state of the art in reliable low power communications for the Industrial Internet of Things (IIoT).
This paper provides experimental evaluations for the end-to-end performance of 6TiSCH on top of 3 physical (PHY) layers : 
    1) the standard 250~kbps Offset-Quadrature Phase Shift Keying (O-QPSK) at 2.4~GHz frequency band, 
    2) 50~kbps bit-rate Frequency Shift Keying (FSK) at 868~MHz band, and 
    3) 800~kbps bit-rate Orthogonal Frequency Division Multiplexing at 868~MHz band.
Experiments are conducted on OpenWSN -- a reference architecture for 6TiSCH, in a setting comparable to an IIoT deployment. 
Performance of the network on top of each PHY is evaluated in terms of 4 Key Performance Indicators (KPIs): 
    formation time, 
    end-to-end reliability,
    end-to-end latency, and
    battery lifetime. 
Results demonstrate that the high link budget of \fsk~ lead to best formation time, reliability, and latency; yet it shorten battery life time by a factor of 10 compared to \oqpsk. 
Alternatively, the low link budget of \oqpsk~ demonstrate slowest formation time, and least reliability, and latency performance; yet it leads to the best energy saving among the three options.
\ofdm~ performance is a compromise between both options, although it is 3 times the bit-rate of \oqpsk; yet it shortens the battery lifetime by a factor of 5 compared to  \oqpsk. 
Therefore, there is no single PHY layer that is best across all KPIs which motivates research in a multi-PHY 6TiSCH stack.
}
...
This section summarizes our key findings, focusing on
    network formation time (Section~\ref{sec:results_formation}),
    end-to-end reliability (Section~\ref{sec:results_reliability}),
    end-to-end latency (Section~\ref{sec:results_latency}), and
    battery lifetime (Section~\ref{sec:results_lifetime}).

%------------------------------------------------------------------------------
\subsection{Network Formation Time}
\label{sec:results_formation}
...
As shown in Section~\ref{sec:results_formation}, a lower link budget leads to higher DAG rank, and more relaying, which increases the likelihood of filling up the packet buffer and dropping packets.
...
\begin{table}
    ...
    \begin{tabular}{|r|r|r|r|r|}
        \hline
                & 90\% network formation & battery lifetime & 90\% end-to-end latency  & end-to-end reliability \\
                & (Section~\ref{sec:results_formation}) & (Section~\ref{sec:results_lifetime}) & (Section~\ref{sec:results_latency})  & (Section~\ref{sec:results_reliability}) \\ \hline
        \fsk    &                  7~min &        1.6~years &                     10~s &               100.00\% \\ \hline
        \ofdm   &                  9~min &        6.9~years &                     25~s &                99.84\% \\ \hline
        \oqpsk  &                 11~min &       15.1~years &                     35~s &                98.08\% \\ \hline
    \end{tabular}
